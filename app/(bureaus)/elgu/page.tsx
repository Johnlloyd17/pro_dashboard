...
export default async function ElguPage({
  searchParams,
}: {
  searchParams: Promise<{ page?: string; year?: string; semester?: string }>;
}) {
  const params = await searchParams;
  const stats = await getActivityStats('ELGU', params.year, params.semester);
  ...
  return (
    <main className='flex flex-col gap-4'>
      <div className='flex flex-row justify-between items-end'>
        <div>
          <CardTitle className='text-xl'>ELGU Metrics</CardTitle>
          <CardDescription>
            Track and manage Electronic Local Government Unit activities and
            performance across municipalities.
          </CardDescription>
        </div>
        <div className='flex flex-row gap-2'>
          <ViewTargets bureauName='ELGU' />
          <TargetsDialog />
          <FilterTerm />
        </div>
      </div>
      ...
      <Tabs defaultValue='overview' className='w-full col-span-4'>
        ...
        <TabsContent value='overview'>
          <DataTableProjects bureauName='ELGU' searchParams={params} />
        </TabsContent>
