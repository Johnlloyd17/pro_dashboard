...
export default async function EgovPage({
  searchParams,
}: {
  searchParams: Promise<{ page?: string; year?: string; semester?: string }>;
}) {
  const params = await searchParams;
  const stats = await getActivityStats('EGOV', params.year, params.semester);
  ...
  return (
    <main className='flex flex-col gap-4'>
      <div className='flex flex-row justify-between items-end'>
        <div>
          <CardTitle className='text-xl'>EGOV Metrics</CardTitle>
          <CardDescription>
            Monitor your organization&apos;s performance and compliance.
          </CardDescription>
        </div>
        <div className='flex flex-row gap-2'>
          <ViewTargets bureauName='EGOV' />
          <TargetsDialog />
          <FilterTerm />
        </div>
      </div>
      ...
      <Tabs defaultValue='overview' className='w-full col-span-4'>
        ...
        <TabsContent value='overview'>
          <DataTableProjects bureauName='EGOV' searchParams={params} />
        </TabsContent>
