...
export default async function GovnetPage({
  searchParams,
}: {
  searchParams: Promise<{ page?: string; year?: string; semester?: string }>;
}) {
  const params = await searchParams;
  const stats = await getActivityStats('GOVNET', params.year, params.semester);
  ...
  return (
    <main className='flex flex-col gap-4'>
      <div className='flex flex-row justify-between items-end'>
        <div>
          <CardTitle className='text-xl'>GOVNET Metrics</CardTitle>
          <CardDescription>
            Monitor government network connectivity and infrastructure performance
            across municipalities.
          </CardDescription>
        </div>
        <div className='flex flex-row gap-2'>
          <ViewTargets bureauName='GOVNET' />
          <TargetsDialog />
          <FilterTerm />
        </div>
      </div>
      ...
      <Tabs defaultValue='overview' className='w-full col-span-4'>
        ...
        <TabsContent value='overview'>
          <DataTableProjects bureauName='GOVNET' searchParams={params} />
        </TabsContent>
