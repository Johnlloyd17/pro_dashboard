...
export default async function CsbPage({
  searchParams,
}: {
  searchParams: Promise<{ page?: string; year?: string; semester?: string }>;
}) {
  const params = await searchParams;
  const stats = await getActivityStats('CSB', params.year, params.semester);
  ...
  return (
    <main className='flex flex-col gap-4'>
      <div className='flex flex-row justify-between items-end'>
        <div>
          <CardTitle className='text-xl'>CSB Metrics</CardTitle>
          <CardDescription>
            Monitor your organization&apos;s performance and compliance.
          </CardDescription>
        </div>
        <div className='flex flex-row gap-2'>
          <ViewTargets bureauName='CSB' />
          <TargetsDialog />
          <FilterTerm />
        </div>
      </div>
      ...
      <Tabs defaultValue='overview' className='w-full col-span-4'>
        ...
        <TabsContent value='overview'>
          <DataTableProjects bureauName='CSB' searchParams={params} />
        </TabsContent>
